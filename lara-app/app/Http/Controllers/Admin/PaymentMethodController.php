...
class PaymentMethodController extends Controller
{
     ...
     * )
     */
    public function linkPaymentMethod(LinkPaymentMethodRequest $request, $applicant_id) {

        $applicant = User::find($applicant_id);
        if(!($applicant instanceof User) || $applicant->status !== UserRoleEnum::Applicant) {
            return response(['message' => 'Applicant not found!'],404);
        }


        // Check whether is possible to move $payment_method_id in parameter instead of request body. For this, LinkedMEthodRequest must be checked after the updating linked method is merged on main.
