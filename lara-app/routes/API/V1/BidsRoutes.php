...
use App\Http\Controllers\API\V1\Bids\User\UpdateBidController;
...
Route::middleware('auth:sanctum')->name('users.bids.')->prefix('users/bids')->group(function () {
   ...
   // Route::middleware('can:view,request')->get('/{request}', ShowRequestController::class)->name('show');
    Route::middleware('can:create,bid')->post('/', StoreBidController::class)->name('store');
    Route::middleware('can:update,bid')->patch('/{bid}',UpdateBidController::class)->name('update');
